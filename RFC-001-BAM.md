...
In COMIT, nodes need to exchange all kinds of information in a peer-to-peer manner. As COMIT is an inherently distributed system, the network will be very heterogeneous at some point, with all kinds of different languages and versions communicating with each other. Given that, we were looking for a transport protocol that fulfills the following requirements:

- **Different kinds of messages:** Not each message is necessarily a request (and thus implies an answer). The protocol needs to be able express messages without responses.
- **Self-descriptive messages:** Because of the heterogeneous nature, the protocol should be evolvable to allow introduction of new features without breaking older versions. Self-descriptive messages help greatly with that because older versions can still parse the message and continue even if they don't fully understand the message. (Spoiler: There are also ways to make older versions break on purpose if they don't fully understand a message.)
- **Dynamic/Polymorphic messages:** Similar to HTTP, the messages sent to the other party don't always take the exact same form although they convey the same meaning. GET requests for example always describe the intent of retrieving a resource. The concrete structure of two GET requests might differ though. Similar to that, we need to send messages between COMIT nodes that communicate a certain intent (like initiate an atomic swap) but might take different structure (based on the involved ledger's for example, different information needs to be exchange).
