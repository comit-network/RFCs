...
This RFC defines how to execute a [RFC003](RFC-003-SWAP-Basic.adoc) SWAP where one of the ledgers is Ethereum and the associated asset is an ERC20 token.
...
The parameters for the ERC20 HTLC follow [RFC003](RFC-003-SWAP-Basic.adoc#hash-time-lock-contract-htlc) and are described concretely in the following table:
...
The following shows an [RFC003](RFC-003-SWAP-Basic.adoc) SWAP REQUEST where the `alpha_ledger` is Ethereum, the `alpha_asset` is 1 PAY token (with `...` being used where the value is only relevant for the `beta_ledger`).
