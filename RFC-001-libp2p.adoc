...
A substream is a virtual socket for a specific protocol between two nodes.
...
TODO: include pretty printing note
...
Headers is a JSON-object with each entry being a single header.
Each header encodes the following information:

1. The "variant" (MUST understand vs MAY ignore)
2. A value
3. Parameters
